...
pxclkv = ueye.INT(40)
exp_time = ueye.DOUBLE(100)
gain_factor = ueye.INT(0)

cal = 0
#---------------------------------------------------------------------------------------------------------------------------------------
# Graph Set-Up

figx = Figure(figsize=(6, 4), dpi=200)
canvasx = FigureCanvas(figx)

figy = Figure(figsize=(6, 4), dpi=200)
...
cross = io.imread(r'bitmaps\bigcross.bmp')
...
while(nRet == ueye.IS_SUCCESS):
    ...
    yxm = np.squeeze(np.mean(frame[int(height.value/2)-20:int(height.value/2)+20,int(width.value/2)-101:int(width.value/2)+99],axis = 0))
    yxp = np.squeeze(np.mean(frame[int(height.value/2)-20:int(height.value/2)+20,int(width.value/2)-99:int(width.value/2)+101],axis = 0))
    yxn[:,4] = yxn[:,3]
    yxn[:,3] = yxn[:,2]
    yxn[:,2] = yxn[:,1]
    yxn[:,1] = yxn[:,0]
    ...
    yym = np.squeeze(np.mean(frame[int(height.value/2)-101:int(height.value/2)+99,int(width.value/2)-20:int(width.value/2)+20],axis = 1))
    yyp = np.squeeze(np.mean(frame[int(height.value/2)-99:int(height.value/2)+101,int(width.value/2)-20:int(width.value/2)+20],axis = 1))
    yyn[:,4] = yyn[:,3]
    yyn[:,3] = yyn[:,2]
    yyn[:,2] = yyn[:,1]
    yyn[:,1] = yyn[:,0]
    ...
    # graphimgx = np.fromstring(canvasx.tostring_rgb(), dtype='uint8').reshape(800, 1200, 3)
    ...
    # graphimgy = np.rot90(np.fromstring(canvasy.tostring_rgb(), dtype='uint8').reshape(800, 1200, 3),k=3)
    ...
    fy, fx, fc = frame.shape
    #print(frame)
    frame2 = np.dstack((frame,frame,frame))
    
    redx = graphimgx[:,:,1] < 160
    graphimgx = frame2[0:800,int(width.value/2)-600:int(width.value/2)+600]
    graphimgx[redx] = [0,0,255]

    redy = graphimgy[:,:,1] < 160
    graphimgy =  frame2[int(height.value/2)-600:int(height.value/2)+600,-800:]
    graphimgy[redy] = [0,0,255]
    
    frame2[0:800,int(width.value/2)-600:int(width.value/2)+600] = graphimgx
    frame2[int(height.value/2)-600:int(height.value/2)+600,-800:] = graphimgy
