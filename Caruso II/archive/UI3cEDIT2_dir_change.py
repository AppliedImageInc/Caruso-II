...
def check_serial_status(ser):
	serial_status = ser.is_open
...
def read_data():
	...
	if data1:
		print("Laser (?) Serial1: " + data1.decode('utf-8'))
	if data2:
		for i in range(len(data2)):
			print("Motor (?) Serial2: " + data2[i].decode('utf-8'))
	if data3:
		print("Controller (?) Serial3: " + data3.decode('utf-8'))


def send_serial_command(command):
	check_serial_status(ser2) # make sure the serial port is open
	ser2.write(bytes(command + '\n', "utf-8"))
	read_data()

def liza_test():
	check_serial_status(ser1)
	check_serial_status(ser2)
...
def parsepart(file):
	...
	centers = [] # written in the program as: C(#),P(#),P(#), midpoints between two points (P)
	meas = [] # written in the program as: M(#),P(#),P(#), distance between two points (P) or centers (C)
	diam = [] # written in the program as: D(X(#)), diameter of a circle measurements
	xy = [] # written in the program as: X(#),(X-loc),(Y-loc),(Radius), center point of a circle
	...
	return(points, centers, meas, diam, xy)
...
def measurepart():
	...
	points, centers, meas, diam, xy = parsepart(file)
	cpoints = {}
	dmeas = {}
	...
	for i in range(len(xy)):
		xypoints = xy[i][1:-1] # each circle point line of the file, but without the first column, ex. just (X-loc),(Y-loc),(Radius)
		name = xy[i][0] # first column, ex. just X#
		xc = float(xypoints[0]) # (X-loc)
		yc = float(xypoints[1]) # (Y-loc)
		r = float(xypoints[2]) # (Radius)
		# if an edge detection method is specified, it will be in the 4th column of the program
		try:
			elo_option = xypoints[3]
		except: 
			elo_option = 'e' # default edge detection method

		# add the top, bottom, left and right of the circle to the list of points 
		points.append([name + 'x1', str(xc - r), str(yc),'x', elo_option])
		points.append([name + 'x2', str(xc + r), str(yc),'x', elo_option])
		points.append([name + 'y1', str(xc), str(yc - r),'y', elo_option])
		points.append([name + 'y2', str(xc), str(yc + r),'y', elo_option])
	...
	for i in range(len(xy)):
		...
		edgedata[0,0] = cpoints[xy[i][0] + 'x1'][0]
		edgedata[0,1] = cpoints[xy[i][0] + 'x1'][1]
		edgedata[1,0] = cpoints[xy[i][0] + 'x2'][0]
		edgedata[1,1] = cpoints[xy[i][0] + 'x2'][1]
		edgedata[2,0] = cpoints[xy[i][0] + 'y1'][0]
		edgedata[2,1] = cpoints[xy[i][0] + 'y1'][1]
		edgedata[3,0] = cpoints[xy[i][0] + 'y2'][0]
		edgedata[3,1] = cpoints[xy[i][0] + 'y2'][1]
		...
		xymeas.append([xy[i][0], xtc, ytc])
		dmeas.update({'D' +xy[i][0]: D})
		cpoints.update({'P'+xy[i][0]:[xtc,ytc]})
	...
	measurement_file = "S:\\Caruso II\\"  + file[:-4] + '-' + sn + '.csv'
	...
	if os.path.isfile(measurement_file):
		...
		with open(measurement_file, 'w') as doc:
			...
			for i in range(len(xymeas)):
				if units == 1:
					doc.write(str(xymeas[i][0]) + ',' + str(round(xymeas[i][1],5)) + ',' + str(round(xymeas[i][2],5)) + ',\n')
				else:
					doc.write(str(xymeas[i][0]) + ',' + str(round(xymeas[i][1]/25.4,6)) + ',' + str(round(xymeas[i][2]/25.4,6)) +  ',\n')
...
# pass in a whole line from the program				
def autocpoint(mline):
	global autostate, pY, pX, cY, cX
	X = float(mline[1])
	Y = float(mline[2])
	xy = mline[3] # which axis NOT diameter point should probably change one or both names
	el = mline[4]
...
liza_test_button = ttk.Button(autoframe, text = 'Ready?', command = liza_test)
...
send_button = ttk.Button(autoframe, text = 'Send Command', command = lambda:[send_serial_command(serial_entry.get())])
