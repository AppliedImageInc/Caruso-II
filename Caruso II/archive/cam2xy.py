...
cross = io.imread('bigercross.bmp')
cross = cross[:,:,0]
...
while(nRet == ueye.IS_SUCCESS):
    ...
    xtm = np.squeeze(np.mean(frame[int(height.value/2)+222:int(height.value/2)+334,int(width.value/2)-111:int(width.value/2)+111],axis = 0))
    xtp = np.squeeze(np.mean(frame[int(height.value/2)+222:int(height.value/2)+334,int(width.value/2)-109:int(width.value/2)+113],axis = 0))
    xtn[:,4] = xtn[:,3]
    xtn[:,3] = xtn[:,2]
    xtn[:,2] = xtn[:,1]
    xtn[:,1] = xtn[:,0]
    xtn[:,0] = np.absolute((xtp-xtm)/2)
    tx = np.squeeze(np.mean(yxn, axis = 1))

    xbm = np.squeeze(np.mean(frame[int(height.value/2)-222:int(height.value/2)-334,int(width.value/2)-111:int(width.value/2)+111],axis = 0))
    xbp = np.squeeze(np.mean(frame[int(height.value/2)-222:int(height.value/2)-334,int(width.value/2)-109:int(width.value/2)+113],axis = 0))
    xbn[:,4] = xbn[:,3]
    xbn[:,3] = xbn[:,2]
    xbn[:,2] = xbn[:,1]
    xbn[:,1] = xbn[:,0]
    xbn[:,0] = np.absolute((xbp-xbm)/2)
    bx = np.squeeze(np.mean(xbn, axis = 1))

    ylm = np.squeeze(np.mean(frame[int(height.value/2)-111:int(height.value/2)+111,int(width.value/2)-222:int(width.value/2)-334],axis = 1))
    ylp = np.squeeze(np.mean(frame[int(height.value/2)-109:int(height.value/2)+113,int(width.value/2)-222:int(width.value/2)-334],axis = 1))
    yln[:,4] = yln[:,3]
    yln[:,3] = yln[:,2]
    yln[:,2] = yln[:,1]
    yln[:,1] = yln[:,0]
    yln[:,0] = np.absolute((ylp-ylm)/2)
    ly = np.squeeze(np.mean(yln, axis = 1))

    yrm = np.squeeze(np.mean(frame[int(height.value/2)-111:int(height.value/2)+111,int(width.value/2)+222:int(width.value/2)+334],axis = 1))
    yrp = np.squeeze(np.mean(frame[int(height.value/2)-109:int(height.value/2)+113,int(width.value/2)+222:int(width.value/2)+334],axis = 1))
    yrn[:,4] = yrn[:,3]
    yrn[:,3] = yrn[:,2]
    yrn[:,2] = yrn[:,1]
    yrn[:,1] = yrn[:,0]
    yrn[:,0] = np.absolute((yrp-yrm)/2)
    ry = np.squeeze(np.mean(yrn, axis = 1))

    cxy = numpy.stack(tx,bx,ly,ry)
