...
while(nRet == ueye.IS_SUCCESS):
    ...
    # graphimgx = np.fromstring(canvasx.tostring_rgb(), dtype='uint8').reshape(800, 2400, 3) # tostring_rgb is deprecated

    axisy.set_ydata(yy)
    canvasy.draw()
    # graphimgy = np.rot90(graphimgy[..., :3])
    graphimgy = np.rot90(np.asarray(canvasx.buffer_rgba())[..., :3].copy().reshape(800, 2400, 3),k=3)
