...
void Example_ReadingRawPackets_GlobalFn(const TLMC_DeviceHandle hDevice);
void __stdcall Example_PacketHandler_GlobalFn(const unsigned char* pBuffer, const size_t bufferLength, const uintptr_t userData);
void Example_ReadingRawPackets_ObjectFn(const TLMC_DeviceHandle hDevice);
void __stdcall Example_PacketHandler_ObjectFn(const unsigned char* pBuffer, const size_t bufferLength, const uintptr_t userData);

void Example_ReceivingNotifications(const TLMC_DeviceHandle hDevice);
void __stdcall Example_NotificationHandlerCallbackFn(const TLMC_Notification* pNotification, const uintptr_t userData);
...
int MAIN_CALL main()
{
    if (TLMC_Startup(nullptr) == TLMC_ResultCodes::TLMC_Success)
    {
        ...
        TLMC_ResultCode resultCode = TLMC_Open("27000128", nullptr, TLMC_OperatingModes::TLMC_OperatingMode_Default, &hDevice);
        ...
        if (resultCode == TLMC_ResultCodes::TLMC_Success)
        {
            ...
            Example_DeactivateCalibration(hDevice);
            Example_Disconnect(hDevice);
            ...
            Example_ReadingRawPackets_GlobalFn(hDevice);
            Example_ReadingRawPackets_ObjectFn(hDevice);
...
void Example_ChangeAnalogMonitorConfigurationParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_AnalogMonitorConfigurationParams params;

    if (TLMC_GetAnalogMonitorConfigurationParams(hDevice, TLMC_AnalogMonitorNumber::TLMC_AnalogueMonitorNumber_1, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.motorChannel = TLMC_AnalogMonitorMotorChannel::TLMC_AnalogMonitorMotorChannel_1;
        params.offset = 0;
        params.scale = 10000;
        params.systemVariable = TLMC_AnalogMonitorSystemVariable::TLMC_AnalogMonitorSystemVariable_PositionError;

        if (TLMC_SetAnalogMonitorConfigurationParams(hDevice, TLMC_AnalogMonitorNumber::TLMC_AnalogueMonitorNumber_1, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeAuxIoPortMode(const TLMC_DeviceHandle hDevice)
{
    TLMC_AuxIoPortMode mode;

    if (TLMC_GetAuxIoPortMode(hDevice, TLMC_AuxIoPortNumber::TLMC_AuxIoPortNumber_Port1, &mode, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // The current auxiliary I/O port mode was read from the controller and
        // copied to 'mode'

        mode = TLMC_AuxIoPortMode::TLMC_AuxIoPortMode_SoftwareControlled;

        if (TLMC_SetAuxIoPortMode(hDevice, TLMC_AuxIoPortNumber::TLMC_AuxIoPortNumber_Port1, mode) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeAuxIoSoftwareStates(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_GetAuxIoSoftwareStates(hDevice, &softwareStates, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        ...
        if (TLMC_SetAuxIoSoftwareStates(hDevice, softwareStates) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeBowIndex(const TLMC_DeviceHandle hDevice)
{
    TLMC_BowIndex bowIndex;

    if (TLMC_GetBowIndex(hDevice, &bowIndex, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // The bow index was read from the controller and copied to bowIndex

        if (TLMC_GetBowIndex(hDevice, &bowIndex, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
        {
    ...
    if (TLMC_SetBowIndex(hDevice, TLMC_BowIndices::TLMC_BowIndex_Trapezoidal) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_ChangeButtonParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_ButtonParams params;

    if (TLMC_GetButtonParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.buttonMode = TLMC_ButtonModes::TLMC_ButtonMode_PresetPosition;
        params.position1 = 0;
        params.position2 = 100;
        params.timeout1Milliseconds = 2000;
        params.timeout2Milliseconds = 2001;

        if (TLMC_SetButtonParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeConnectedProductInfo(const TLMC_DeviceHandle hDevice)
{
    TLMC_ConnectedProductInfo productInfo;

    if (TLMC_GetConnectedProductInfo(hDevice, &productInfo) == TLMC_ResultCodes::TLMC_Success)
    {
        productInfo.productName[0] = 'A';
        productInfo.productName[1] = 'B';
        productInfo.productName[2] = 'C';
        productInfo.productName[3] = '\0';
        productInfo.axisType = TLMC_ConnectedProductAxisTypes::TLMC_ConnectedProductAxisType_Single;
        productInfo.movementType = TLMC_ConnectedProductMovementTypes::TLMC_ConnectedProductMovementType_Linear;
        productInfo.unitType = TLMC_Units::TLMC_Unit_Millimetres;
        ...
        if (TLMC_SetConnectedProductInfo(hDevice, &productInfo) == TLMC_ResultCodes::TLMC_Success)
        {
            // The new connected product information has been set
        }
    }
}
...
void Example_ChangeCurrentLoopParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_CurrentLoopParams params;

    if (TLMC_GetCurrentLoopParams(hDevice, TLMC_CurrentLoopScenarios::TLMC_CurrentLoopScenario_Settled, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.feedForward = 500;
        params.integral = 80;
        params.integralDeadBand = 0;
        params.integralLimit = 32767;
        params.phase = TLMC_CurrentLoopPhases::TLMC_CurrentLoopPhase_AB;
        params.proportional = 20;
        params.scalingFactor = 1.0f;

        if (TLMC_SetCurrentLoopParams(hDevice, TLMC_CurrentLoopScenarios::TLMC_CurrentLoopScenario_Settled, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeDcPidParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_DcPidParams params;

    if (TLMC_GetDcPidParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.proportional = 850;
        params.integral = 150;
        params.derivative = 2720;
        params.integralLimit = 50;
        params.filterControl = TLMC_DcPidUpdateFilters::TLMC_DcPidUpdateFilter_All;

        if (TLMC_SetDcPidParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeDigitalOutputStates(const TLMC_DeviceHandle hDevice)
{
    TLMC_DigitalOutput digitalOutputStates;

    if (TLMC_GetDigitalOutputStates(hDevice, &digitalOutputStates, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        digitalOutputStates = TLMC_DigitalOutputs::TLMC_DigitalOutput_None;

        if (TLMC_SetDigitalOutputStates(hDevice, digitalOutputStates) == TLMC_ResultCodes::TLMC_Success)
        {
...
{
    {
        TLMC_EnableState enableState;

        if (TLMC_GetEnableState(hDevice, &enableState, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
        {
            // The enable state has been read from the controller
        }
    }

    if (TLMC_SetEnableState(hDevice, TLMC_EnableStates::TLMC_Enabled, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_ChangeGeneralMoveParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_GeneralMoveParams params;

    if (TLMC_GetGeneralMoveParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.backlashDistance = 1920;

        if (TLMC_SetGeneralMoveParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeHomeParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_HomeParams params;

    if (TLMC_GetHomeParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.direction = TLMC_HomeDirections::TLMC_HomeDirection_Reverse;
        params.limitSwitch = TLMC_HomeLimitSwitches::TLMC_HomeLimitSwitch_Reverse;
        params.velocity = 428997;
        params.offsetDistance = 7678;

        if (TLMC_SetHomeParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeIoConfigurationParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_IoConfigurationParams params;
    const TLMC_IoPortNumber portNumber = TLMC_IoPortNumber::TLMC_IoPortNumber_Port1;

    if (TLMC_GetIoConfigurationParams(hDevice, portNumber, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.mode = TLMC_IoPortMode::TLMC_IoPortMode_DigitalOutput;
        params.triggerOutSource = TLMC_IoPortSource::TLMC_IoPortSource_Channel1;

        if (TLMC_SetIoConfigurationParams(hDevice, portNumber, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
{
    {
        TLMC_IoPositionTriggerEnableState state;

        if (TLMC_GetIoPositionTriggerEnableState(hDevice, &state, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
        {
            // The position trigger enable state has been read from the controller
        }
    }

    if (TLMC_SetIoPositionTriggerEnableState(hDevice, TLMC_IoPositionTriggerEnableState::TLMC_IoPositionTriggerEnableState_Armed, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_ChangeIoTriggerParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_IoTriggerParams params;

    if (TLMC_GetIoTriggerParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.triggerInMode = TLMC_IoTriggerInMode::TLMC_IoTriggerInMode_TriggersAbsoluteMove;
        params.triggerInPolarity = TLMC_IoTriggerPolarity::TLMC_IoTriggerPolarity_ActiveIsLogicHigh;
        params.triggerInSource = TLMC_IoTriggerInSource::TLMC_IoTriggerInSource_Io1;
        params.triggerOutMode = TLMC_IoTriggerOutMode::TLMC_IoTriggerOutMode_ActiveDuringMotion;
        params.triggerOutPolarity = TLMC_IoTriggerPolarity::TLMC_IoTriggerPolarity_ActiveIsLogicHigh;
        ...
        if (TLMC_SetIoTriggerParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZ_ChangeKpcIoSettingsParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZ_KpcIoSettingsParams params;

    if (TLMC_PZ_GetKpcIoSettingsParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.analogInputSource = TLMC_PZ_KpcAnalogInputSources::TLMC_PZ_KpcAnalogInputSource_HubInputA;
        params.filterCutOffFrequency = 40;
        params.forceSense = 50;
        params.strainGaugeOption = TLMC_PZ_KpcStrainGaugeOptions::TLMC_PZ_KpcStrainGaugeOption_Position;
        params.voltageLimit = 75;
        params.voltageRange = TLMC_PZ_KpcVoltageRanges::TLMC_PZ_KpcVoltageRange_75Volts;

        if (TLMC_PZ_SetKpcIoSettingsParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeJogParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_JogParams params;

    if (TLMC_GetJogParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.mode = TLMC_JogModes::TLMC_JogMode_Continuous;
        params.stepSize = 19196;
        params.acceleration = 219;
        params.maxVelocity = 643495;
        params.stopMode = TLMC_JogStopModes::TLMC_JogStopMode_Immediate;

        if (TLMC_SetJogParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeJoystickParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_JoystickParams params;

    if (TLMC_GetJoystickParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        ...
        params.directionSense = TLMC_JoystickDirectionSenses::TLMC_JoystickDirectionSense_Positive;

        if (TLMC_SetJoystickParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeKcubeIoTriggerParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_KcubeIoTriggerParams params;

    if (TLMC_GetKcubeIoTriggerParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.trigger1Mode = TLMC_KcubeIoTriggerModes::TLMC_KcubeIoTriggerMode_GeneralPurposeInput;
        params.trigger1Polarity = TLMC_KcubeIoTriggerPolarities::TLMC_KcubeIoTriggerPolarity_ActiveIsLogicHigh;
        params.trigger2Mode = TLMC_KcubeIoTriggerModes::TLMC_KcubeIoTriggerMode_GeneralPurposeOutput;
        params.trigger2Polarity = TLMC_KcubeIoTriggerPolarities::TLMC_KcubeIoTriggerPolarity_ActiveIsLogicHigh;

        if (TLMC_SetKcubeIoTriggerParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZ_ChangeKpcIoTriggerParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZ_KpcIoTriggerParams params;

    if (TLMC_PZ_GetKpcIoTriggerParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.trigger1Mode = TLMC_PZ_KpcIoTriggerModes::TLMC_PZ_KpcIoTriggerMode_GeneralPurposeInput;
        params.trigger1Polarity = TLMC_KcubeIoTriggerPolarities::TLMC_KcubeIoTriggerPolarity_ActiveIsLogicHigh;
        params.trigger2Mode = TLMC_PZ_KpcIoTriggerModes::TLMC_PZ_KpcIoTriggerMode_GeneralPurposeOutput;
        params.trigger2Polarity = TLMC_KcubeIoTriggerPolarities::TLMC_KcubeIoTriggerPolarity_ActiveIsLogicHigh;
        params.monitorFilterCutOffFrequency = 20;
        params.monitorOutputMode = TLMC_PZ_KpcMonitorOutputModes::TLMC_PZ_KpcMonitorOutputMode_HighVoltage;
        ...
        if (TLMC_PZ_SetKpcIoTriggerParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeKcubeMmiLockState(const TLMC_DeviceHandle hDevice)
{
    TLMC_KcubeMmiLockState_Type lockState = TLMC_KcubeMmiLockState::TLMC_KcubeMmiLockState_Locked;

    if (TLMC_GetKcubeMmiLockState(hDevice, &lockState, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // The MMI lock state has been loaded from the controller and copied to the
        // lockState variable.

        if (TLMC_GetKcubeMmiLockState(hDevice, &lockState, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
        {
            // The MMI lock state has been loaded from the controller and copied to the
            // lockState variable.
        }
        ...
        if (TLMC_SetKcubeMmiLockState(hDevice, lockState) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeKcubeMmiParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_KcubeMmiParams params;

    if (TLMC_GetKcubeMmiParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.joystickMode = TLMC_KcubeMmiJoystickModes::TLMC_KcubeMmiJoystickMode_ControlsVelocity;
        params.joystickMaxVelocity = 966187;
        params.joystickAcceleration = 131;
        params.joystickDirectionSense = TLMC_KcubeMmiJoystickDirectionSenses::TLMC_KcubeMmiJoystickDirectionSense_Normal;
        ...
        if (TLMC_SetKcubeMmiParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZ_ChangeKpcMmiParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZ_KpcMmiParams params;

    if (TLMC_PZ_GetKpcMmiParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.joystickMode = TLMC_PZ_KpcMmiJoystickModes::TLMC_PZ_KpcMmiJoystickMode_Jogs;
        params.joystickGear = TLMC_KcubeMmiJoystickGears::TLMC_KcubeMmiJoystickGear_High;
        params.joystickPositionStepSize = 40;
        params.joystickVoltageStepSize = 20;
        params.joystickDirectionSense = TLMC_PZ_KpcMmiJoystickDirectionSenses::TLMC_PZ_KpcMmiJoystickDirectionSense_Normal;
        ...
        if (TLMC_PZ_SetKpcMmiParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeKcubePositionTriggerParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_KcubePositionTriggerParams params;

    if (TLMC_GetKcubePositionTriggerParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        ...
        if (TLMC_SetKcubePositionTriggerParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeLcdDisplayParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_LcdDisplayParams params;

    if (TLMC_GetLcdDisplayParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        ...
        if (TLMC_SetLcdDisplayParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeLcdMoveParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_LcdMoveParams params;

    if (TLMC_GetLcdMoveParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.acceleration = 13744;
        params.jogStepSize = 60000;
        params.jogStopMode = TLMC_JogStopMode::TLMC_JogStopMode_Immediate;
        params.knobMode = TLMC_LcdKnobMode::TLMC_LcdKnobMode_Velocity;
        params.maxVelocity = 6710886;
        params.presetPosition[0] = 600000;

        if (TLMC_SetLcdMoveParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeLimitSwitchParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_LimitSwitchParams params;

    if (TLMC_GetLimitSwitchParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.clockwiseHardLimitOperatingMode = TLMC_HardLimitOperatingModes::TLMC_HardLimitOperatingMode_SwitchContactMakesWhenHoming;
        params.counterclockwiseHardLimitOperatingMode = TLMC_HardLimitOperatingModes::TLMC_HardLimitOperatingMode_SwitchIgnored;
        params.clockwiseSoftLimit = 1073741824;
        params.counterclockwiseSoftLimit = -1073741824;
        params.softLimitOperatingMode = TLMC_SoftLimitOperatingModes::TLMC_SoftLimitOperatingMode_Ignored;

        if (TLMC_SetLimitSwitchParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeMotorOutputParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_MotorOutputParams params;

    if (TLMC_GetMotorOutputParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.continuousCurrentLimit = 22937;
        params.energyLimit = 19660;
        params.motorLimit = 32767;

        if (TLMC_SetMotorOutputParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeMoveAbsoluteParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_MoveAbsoluteParams params;

    if (TLMC_GetMoveAbsoluteParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.absolutePosition = 86383;

        if (TLMC_SetMoveAbsoluteParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeMoveRelativeParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_MoveRelativeParams params;

    if (TLMC_GetMoveRelativeParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.relativeDistance = 19196;

        if (TLMC_SetMoveRelativeParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangePositionLoopParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_PositionLoopParams params;

    if (TLMC_GetPositionLoopParams(hDevice, TLMC_PositionLoopScenarios::TLMC_PositionLoopScenario_Stationary, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        ...
        if (TLMC_SetPositionLoopParams(hDevice, TLMC_PositionLoopScenarios::TLMC_PositionLoopScenario_Stationary, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangePotentiometerParams(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_GetPotentiometerParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.deflection0 = 10;
        params.velocity1 = 10290;

        if (TLMC_SetPotentiometerParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangePowerParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_PowerParams powerParams;

    if (TLMC_GetPowerParams(hDevice, &powerParams, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // The power params were retrieved from the controller and copied to powerParams.

        if (TLMC_GetPowerParams(hDevice, &powerParams, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
        {
        ...
        if (TLMC_SetPowerParams(hDevice, &powerParams) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangePresetConnectedProduct(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_SetConnectedProduct(hDevice, "PRM1Z8") == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_ChangeProfileModeParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_ProfileModeParams params;

    if (TLMC_GetProfileModeParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.mode = TLMC_ProfileModes::TLMC_ProfileMode_Trapezoidal;
        params.jerk = 922233;

        if (TLMC_SetProfileModeParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeStageAxisParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_StageAxisParams params;

    if (TLMC_GetStageAxisParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.minPosition = 0;
        params.maxPosition = 100000;

        if (TLMC_SetStageAxisParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeStepperLoopParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_StepperLoopParams stepperLoopParams{};

    if (TLMC_GetStepperLoopParams(hDevice, &stepperLoopParams, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        ...
        stepperLoopParams.loopMode = TLMC_StepperLoopMode::TLMC_StepperLoopMode_Closed;
        stepperLoopParams.proportional = 10000;

        if (TLMC_SetStepperLoopParams(hDevice, &stepperLoopParams) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeTrackSettleParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_TrackSettleParams params;

    if (TLMC_GetTrackSettleParams(hDevice, &params, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.settleTime = 197;
        params.settleWindow = 4;
        params.trackWindow = 0;

        if (TLMC_SetTrackSettleParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeTriggerParamsForDcBrushless(const TLMC_DeviceHandle hDevice)
{
    TLMC_TriggerParamsForDcBrushless params;

    if (TLMC_GetTriggerParamsForDcBrushless(hDevice, &params, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.modes = static_cast<TLMC_TriggerModesForDcBrushless>(
            static_cast<uint32_t>(TLMC_TriggerModesForDcBrushless::TLMC_TriggerModesForDcBrushless_InputActiveIsLogicHigh) |
            static_cast<uint32_t>(TLMC_TriggerModesForDcBrushless::TLMC_TriggerModesForDcBrushless_InputTriggersHomeMove));

        if (TLMC_SetTriggerParamsForDcBrushless(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_ChangeTriggerParamsForStepper(const TLMC_DeviceHandle hDevice)
{
    TLMC_TriggerParamsForStepper triggerParamsForStepper{};

    if (TLMC_GetTriggerParamsForStepper(hDevice, &triggerParamsForStepper, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Stepper trigger params have been loaded from the controller
    }

    if (TLMC_SetTriggerParamsForStepper(hDevice, &triggerParamsForStepper) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_ChangeVelocityParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_VelocityParams params;

    if (TLMC_GetVelocityParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.minVelocity = 0;
        params.acceleration = 146;
        params.maxVelocity = 428997;

        if (TLMC_SetVelocityParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZ_ChangeMaxOutputVoltageParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZ_MaxOutputVoltageParams MaxOutputVoltageParams;

    if (TLMC_PZ_GetMaxOutputVoltageParams(hDevice, &MaxOutputVoltageParams, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        uint16_t maxOutputVoltage = 74;

        if (TLMC_PZ_SetMaxOutputVoltage(hDevice, maxOutputVoltage) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZ_ChangeOutputVoltage(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZ_GetOutputVoltage(hDevice, &outputVoltage, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        outputVoltage = 0;

        if (TLMC_PZ_SetOutputVoltage(hDevice, outputVoltage) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZ_ChangeOutputVoltageControlSourceParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZ_OutputVoltageControlSourceParams outputVoltageControlSourceParams;

    if (TLMC_PZ_GetOutputVoltageControlSourceParams(hDevice, &outputVoltageControlSourceParams, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        outputVoltageControlSourceParams.source = TLMC_PZ_OutputVoltageControlSources::TLMC_PZ_OutputVoltageControlSource_SoftwareOnly;

        if (TLMC_PZ_SetOutputVoltageControlSourceParams(hDevice, &outputVoltageControlSourceParams) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZ_ChangeOutputWaveformParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZ_OutputWaveformParams params;

    if (TLMC_PZ_GetOutputWaveformParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.interSampleDelay = 10;
        params.mode = TLMC_PZ_OutputWaveformOperatingModes::TLMC_PZ_OutputWaveformOperatingMode_Continuous;
        params.numberOfCycles = 0;
        ...
        if (TLMC_PZ_SetOutputWaveformParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZ_ChangePosition(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZ_GetPosition(hDevice, &position, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        position = 0;

        if (TLMC_PZ_SetPosition(hDevice, position) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZ_ChangePositionControlMode(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZ_GetPositionControlMode(hDevice, &positionControlMode, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        positionControlMode = TLMC_PZ_PositionControlModes::TLMC_PZ_PositionControlMode_OpenLoop;

        if (TLMC_PZ_SetPositionControlMode(hDevice, positionControlMode, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZ_ChangePositionLoopParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZ_PositionLoopParams params;

    if (TLMC_PZ_GetPositionLoopParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.integral = 100;
        params.proportional = 100;

        if (TLMC_PZ_SetPositionLoopParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZ_ChangeSlewRateParams(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZ_SlewRateParams params;

    if (TLMC_PZ_GetSlewRateParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.closedLoopSlewRate = 200;
        params.openLoopSlewRate = 200;

        if (TLMC_PZ_SetSlewRateParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZIM_ChangeAbnormalMoveDetectionParams(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZIM_GetAbnormalMoveDetectionParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        if (params.state == TLMC_PZIM_AbnormalMoveDetectionState_Type::TLMC_PZIM_AbnormalMoveDetectionState_Disabled)
        {
            params.state = TLMC_PZIM_AbnormalMoveDetectionState_Type::TLMC_PZIM_AbnormalMoveDetectionState_Enabled;
        }
        else
        {
            params.state = TLMC_PZIM_AbnormalMoveDetectionState_Type::TLMC_PZIM_AbnormalMoveDetectionState_Disabled;
        }

        if (TLMC_PZIM_SetAbnormalMoveDetectionParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZIM_ChangeAmplifierOutputParams(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZIM_GetAmplifierOutputParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.forwardAmplifierLevel -= 1;

        if (TLMC_PZIM_SetAmplifierOutputParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZIM_ChangeClosedLoopMoveParams(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZIM_GetClosedLoopMoveParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.desiredPosition -= 1;

        if (TLMC_PZIM_SetClosedLoopMoveParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZIM_ChangeClosedLoopParams(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZIM_GetClosedLoopParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.referenceSpeed -= 1;

        if (TLMC_PZIM_SetClosedLoopParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZIM_ChangeEthernetParams(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZIM_GetEthernetParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        if (params.ipAddressAssignmentMode == TLMC_PZIM_IpAddressAssignmentModes::TLMC_PZIM_IpAddressAssignmentMode_FromDHCP)
        {
            params.ipAddressAssignmentMode = TLMC_PZIM_IpAddressAssignmentModes::TLMC_PZIM_IpAddressAssignmentMode_FromParameters;
        }
        else
        {
            params.ipAddressAssignmentMode = TLMC_PZIM_IpAddressAssignmentModes::TLMC_PZIM_IpAddressAssignmentMode_FromDHCP;
        }
        if (TLMC_PZIM_SetEthernetParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZIM_ChangeExternalTriggerConfig(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZIM_ExternalTriggerConfig externalTriggerConfig;
    if (TLMC_PZIM_GetExternalTriggerConfig(hDevice, &externalTriggerConfig, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        externalTriggerConfig.mode = TLMC_PZIM_ExternalTriggerModes::TLMC_PZIM_ExternalTriggerMode_AnalogInWithRisingTriggerEdge;
        if (TLMC_PZIM_SetExternalTriggerConfig(hDevice, &externalTriggerConfig) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZIM_ChangeExternalTriggerParams(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZIM_GetExternalTriggerParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.risingEdgeRelativeDistance += 1;
        if (TLMC_PZIM_SetExternalTriggerParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_PZIM_ChangeJogParams(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZIM_GetJogParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.openLoopStepSize -= 1;

        if (TLMC_PZIM_SetJogParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
            // The new jog parameters have been sent to the controller
        }
    }
...
void Example_PZIM_ChangeOpenLoopMoveParams(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZIM_GetOpenLoopMoveParams(hDevice, &params, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        params.targetStepSize -= 1;

        if (TLMC_PZIM_SetOpenLoopMoveParams(hDevice, &params) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_GetAllSettings(const TLMC_DeviceHandle hDevice)
{
...

    {
        ...
        if (TLMC_GetSettingCount(hDevice, &numberOfItemsAvailable) == TLMC_ResultCodes::TLMC_Success) // Finds out how many settings items are available on this device
        {
            // Preallocate a vector large enough to hold all of the settings items
            std::vector<TLMC_Setting> itemsBlock(numberOfItemsAvailable);
            uint16_t numberOfItemsCopied = 0;

            // Request 'numberOfItemsAvailable' items starting at the first one (item zero).
            if ((TLMC_GetSettings(hDevice, 0, numberOfItemsAvailable, itemsBlock.data(), &numberOfItemsCopied) == TLMC_ResultCodes::TLMC_Success) &&
                (numberOfItemsCopied == numberOfItemsAvailable))
            {
                // 'itemsBlock' now contains a copy of all settings items.
    ...
    {
        const size_t numberOfItemsPerCall = 5;
        TLMC_Setting settingsItems[numberOfItemsPerCall];
        uint16_t numberOfItemsCopied = 0;
        uint16_t sourceStartItemIndex = 0;

        // Repeatedly call TLMC_GetSettings until no more items are available
        while (TLMC_GetSettings(hDevice, sourceStartItemIndex, numberOfItemsPerCall, settingsItems, &numberOfItemsCopied) == TLMC_ResultCodes::TLMC_Success)
        {
            ...
            if (numberOfItemsCopied < numberOfItemsPerCall)
            {
                break; // No more items
            }

            // Increase the 'startItemIndex' for the next call
            sourceStartItemIndex += numberOfItemsPerCall;
        }
    }
}
...
void Example_GetAllSettingsAsString(const TLMC_DeviceHandle hDevice)
{
...

    {
        ...
        if (TLMC_GetSettingsAsString(hDevice, nullptr, 0, &charactersExpected, TLMC_SettingStringFormats::TLMC_SettingStringFormat_Json, true) == TLMC_ResultCodes::TLMC_Success) // Obtain the buffer length required
        {
            ...
            if (pBuffer != nullptr)
            {
                ...
                if (TLMC_GetSettingsAsString(hDevice, pBuffer, bufferSize, &charactersCopied, TLMC_SettingStringFormats::TLMC_SettingStringFormat_Json, true) == TLMC_ResultCodes::TLMC_Success)
                {
...
void Example_GetAllStatus(const TLMC_DeviceHandle hDevice)
{
...

    {
        ...
        if (TLMC_GetStatusItemCount(hDevice, &numberOfItemsAvailable) == TLMC_ResultCodes::TLMC_Success) // Finds out how many status items are available on this device
        {
            // Preallocate a vector large enough to hold all of the status items
            std::vector<TLMC_StatusItem> itemsBlock(numberOfItemsAvailable);
            uint16_t numberOfItemsCopied = 0;

            // Request 'numberOfItemsAvailable' items starting at the first one (item zero).
            if ((TLMC_GetStatusItems(hDevice, 0, numberOfItemsAvailable, itemsBlock.data(), &numberOfItemsCopied) == TLMC_ResultCodes::TLMC_Success) &&
                (numberOfItemsCopied == numberOfItemsAvailable))
            {
                // 'itemsBlock' now contains a copy of all status items
    ...
    {
        const size_t numberOfItemsPerCall = 5;
        TLMC_StatusItem statusItems[numberOfItemsPerCall];
        uint16_t numberOfItemsCopied = 0;
        uint16_t sourceStartItemIndex = 0;

        // Repeatedly call TLMC_GetStatusItems until no more items are available
        while (TLMC_GetStatusItems(hDevice, sourceStartItemIndex, numberOfItemsPerCall, statusItems, &numberOfItemsCopied) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_GetApiVersion()
{
    TLMC_ApiVersion apiVersion;

    if (TLMC_GetApiVersion(&apiVersion) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetCalibrationState(const TLMC_DeviceHandle hDevice)
{
    TLMC_CalibrationState state;

    if (TLMC_GetCalibrationState(hDevice, &state) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetConnectedProductsSupported(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_GetConnectedProductsSupported(hDevice, nullptr, 0, &charactersExpected) == TLMC_ResultCodes::TLMC_Success)
    {
        ...
        if (pBuffer != NULL)
        {
            ...
            if (TLMC_GetConnectedProductsSupported(hDevice, pBuffer, bufferSize, &charactersCopied) == TLMC_ResultCodes::TLMC_Success)
            {
...
void Example_GetDeviceInfo(const TLMC_DeviceHandle hDevice)
{
    TLMC_DeviceInfo deviceInfo;

    if (TLMC_GetDeviceInfo(hDevice, &deviceInfo) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetDeviceList()
{
    ...
    if (TLMC_GetDeviceListItemCount(&deviceCount) == TLMC_ResultCodes::TLMC_Success) // Finds out how many device list entries are available
    {
        std::vector<TLMC_DeviceInfo> deviceInfoBlock(deviceCount);
        uint16_t numberOfItemsCopied = 0;

        // Request all items starting with the first one (index zero).
        if ((TLMC_GetDeviceListItems(0, deviceCount, deviceInfoBlock.data(), &numberOfItemsCopied) == TLMC_ResultCodes::TLMC_Success) &&
            (numberOfItemsCopied == deviceCount))
        {
            // 'deviceInfoBlock' now contains a copy of all device list entries
...
void Example_GetDigitalInputStates(const TLMC_DeviceHandle hDevice)
{
    TLMC_DigitalInput state;

    if (TLMC_GetDigitalInputStates(hDevice, &state, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetEncoderCounter(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_GetEncoderCounter(hDevice, &encoderCounter, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // The encoder counter was read from the controller and copied to encoderCounter

        if (TLMC_GetEncoderCounter(hDevice, &encoderCounter, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_GetAdcInputs(const TLMC_DeviceHandle hDevice)
{
    TLMC_AdcInputs state{};

    if (TLMC_GetAdcInputs(hDevice, &state, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Digital input states have been loaded from the controller
    }
}
...
void Example_GetFirmwareVersionInfo(const TLMC_DeviceHandle hDevice)
{
    TLMC_FirmwareVersion deviceFirmwareVersion;
    TLMC_FirmwareVersion requiredFirmwareVersion;

    if (TLMC_GetFirmwareVersionInfo(hDevice, &deviceFirmwareVersion, &requiredFirmwareVersion) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetHardwareInfo(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_GetHardwareInfo(hDevice, &hardwareInfo, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Hardware information has been loaded from the controller and copied to the
        // hardwareInfo structure
    }

    if (TLMC_GetHardwareInfo(hDevice, &hardwareInfo, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetIoConfigurationNumberOfPortsSupported(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_GetIoConfigurationNumberOfPortsSupported(hDevice, &numberOfPorts) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetPositionCounter(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_GetPositionCounter(hDevice, &positionCounter, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // The position counter was read from the controller and copied to positionCounter

        if (TLMC_GetPositionCounter(hDevice, &positionCounter, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_GetRackBayOccupiedState(const TLMC_DeviceHandle hDevice)
{
    TLMC_RackBayOccupiedState state;
    TLMC_RackBayNumber bayNumber = TLMC_RackBayNumbers::TLMC_RackBayNumber_1;

    if (TLMC_GetRackBayOccupiedState(hDevice, bayNumber, &state, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetSetting(const TLMC_DeviceHandle hDevice)
{
    ...
    TLMC_Setting settingItem;

    if (TLMC_GetSetting(hDevice, "home.offsetDistance", &settingItem, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
    ...
    if (TLMC_GetSetting(hDevice, "home.offsetDistance", &settingItem, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetStatus(const TLMC_DeviceHandle hDevice)
{
    ...
    TLMC_StatusItem statusItem;

    if (TLMC_GetStatusItem(hDevice, TLMC_StatusItemIds::TLMC_StatusItemId_Position, &statusItem) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetStepperStatus(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_GetStepperStatus(hDevice, &stepperStatus, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Stepper status data has been loaded from the controller and copied to the
        // stepperStatus variable.

        if (TLMC_GetStepperStatus(hDevice, &stepperStatus, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
        {
...
void Example_GetUmcStatus(const TLMC_DeviceHandle hDevice)
{
    TLMC_UmcStatus umcStatus;

    if (TLMC_GetUmcStatus(hDevice, &umcStatus, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // UMC (Universal Motor Controller) status data has been loaded from the controller and copied to the
        // umcStatus variable
    }

    if (TLMC_GetUmcStatus(hDevice, &umcStatus, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetUniversalStatus(const TLMC_DeviceHandle hDevice)
{
    TLMC_UniversalStatus universalStatus;

    if (TLMC_GetUniversalStatus(hDevice, &universalStatus, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Universal status data has been loaded from the controller and copied to the
        // universalStatus variable
    }

    if (TLMC_GetUniversalStatus(hDevice, &universalStatus, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetUniversalStatusBits(const TLMC_DeviceHandle hDevice)
{
    TLMC_UniversalStatusBits universalStatusBits;

    if (TLMC_GetUniversalStatusBits(hDevice, &universalStatusBits, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Universal status bits have been loaded from the controller and copied to
        // universalStatusBits
    }

    if (TLMC_GetUniversalStatusBits(hDevice, &universalStatusBits, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_PZ_GetMaxTravel(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZ_GetMaxTravel(hDevice, &maxTravel, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Piezo maximum travel was read the controller and copied to maxTravel
    }

    if (TLMC_PZ_GetMaxTravel(hDevice, &maxTravel, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_PZ_GetStatus(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZ_Status piezoStatus;

    if (TLMC_PZ_GetStatus(hDevice, &piezoStatus, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Piezo status data has been loaded from the controller and copied to the
        // piezoStatus structure
    }

    if (TLMC_PZ_GetStatus(hDevice, &piezoStatus, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_PZIM_GetCurrentPosition(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZIM_GetCurrentPosition(hDevice, &position, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
    }
}
...
void Example_PZIM_GetStatus(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZIM_Status piezoInertialMotorStatus;

    if (TLMC_PZIM_GetStatus(hDevice, &piezoInertialMotorStatus, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Piezo inertial motor status data has been loaded from the controller and copied to the
        // piezoInertialMotorStatus structure
    }

    if (TLMC_PZIM_GetStatus(hDevice, &piezoInertialMotorStatus, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_PZIM_GetTriggerTargetPosition(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_PZIM_GetTriggerTargetPosition(hDevice, &position, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
    }
}
...
void Example_SetEncoderCounter(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_SetEncoderCounter(hDevice, 1000) == TLMC_ResultCodes::TLMC_Success)
    {
        // A request to set the encoder counter to the value specified has been sent to the controller
    }

    if (TLMC_SetEncoderCounter(hDevice, 0) == TLMC_ResultCodes::TLMC_Success)
    {
        // A request to zero the encoder counter has been sent to the controller
    }
}

void Example_SetEndOfMoveMessagesMode(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_SetEndOfMoveMessagesMode(hDevice, TLMC_EndOfMoveMessagesModes::TLMC_EndOfMoveMessagesMode_Disabled) == TLMC_ResultCodes::TLMC_Success)
    {
        // A request has been sent to the controller instructing it to stop sending end of move messages.
    }

    if (TLMC_SetEndOfMoveMessagesMode(hDevice, TLMC_EndOfMoveMessagesModes::TLMC_EndOfMoveMessagesMode_Enabled) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_SetPositionCounter(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_SetPositionCounter(hDevice, 1000) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_SetSetting(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_SetSetting(hDevice, "home.offsetDistance", &newValue) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_SetSettingsFromString(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_SetSettingsFromString(hDevice, "{\"controlLoops.current.proportional\" : 20, \"io1.mode\" : 0, \"io1.polarity\" : 2") == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_SetStatusMode(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_SetStatusMode(hDevice, TLMC_OperatingModes::TLMC_OperatingMode_ManualStatusPolling) == TLMC_ResultCodes::TLMC_Success)
    {
    ...
    if (TLMC_SetStatusMode(hDevice, TLMC_OperatingModes::TLMC_OperatingMode_StatusPushedByController) == TLMC_ResultCodes::TLMC_Success)
    {
        // A request has been sent to the controller instructing it to start sending regular status updates.
        // Status update requests will not automatically be sent to the controller.
    }

    if (TLMC_SetStatusMode(hDevice, TLMC_OperatingModes::TLMC_OperatingMode_AutomaticStatusPolling) == TLMC_ResultCodes::TLMC_Success)
    {
        // A request has been sent to the controller instructing it to stop sending regular status updates.
...
void Example_PZ_SetOutputWaveformLookupTableSample(const TLMC_DeviceHandle hDevice)
{
    TLMC_PZ_OutputWaveformLookupTableSample sample;

    sample.index = 0;
    sample.voltage = 0;

    if (TLMC_PZ_SetOutputWaveformLookupTableSample(hDevice, &sample) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_PZ_SetZero(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_PZ_SetZero(hDevice, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Zeroing of the controller completed successfully
    }

    if (TLMC_PZ_SetZero(hDevice, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_ActivateCalibration(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_ActivateCalibration(hDevice) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_DeactivateCalibration(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_DeactivateCalibration(hDevice) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_Disconnect(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_Disconnect(hDevice) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_Home(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_Home(hDevice, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Homing of the actuator completed successfully
    }

    if (TLMC_Home(hDevice, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_Identify(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_Identify(hDevice) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_LoadParams(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_LoadParams(hDevice) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_MoveAbsolute(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_Move(hDevice, TLMC_MoveModes::TLMC_MoveMode_AbsoluteToProgrammedPosition, TLMC_Unused, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // The stage has been move to the preprogrammed absolute position (previously set with MoveAbsoluteParams)
    }

    if (TLMC_Move(hDevice, TLMC_MoveModes::TLMC_MoveMode_Absolute, (int)(((double)45.0) * ((double)1919.6418578623391)), TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_MoveContinuous(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_Move(hDevice, TLMC_MoveModes::TLMC_MoveMode_ContinuousForward, TLMC_Unused, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_MoveJog(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_Move(hDevice, TLMC_MoveModes::TLMC_MoveMode_JogReverse, TLMC_Unused, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_MoveRelative(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_Move(hDevice, TLMC_MoveModes::TLMC_MoveMode_RelativeByProgrammedDistance, TLMC_Unused, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // The stage has been moved by the preprogrammed relative distance (previously set with MoveRelativeParams)
    }

    if (TLMC_Move(hDevice, TLMC_MoveModes::TLMC_MoveMode_Relative, (int)(((double)45.0) * ((double)1919.6418578623391)), TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_MoveSyncArray(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_SetMoveSyncArray(hDevice, &moveSyncArray) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_MoveSyncParams(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_SetMoveSyncParams(hDevice, &moveSyncParams) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_MoveSyncStart(const TLMC_DeviceHandle hDevice)
{
    ...
    moveSyncStartParams.trigger = TLMC_MoveSyncStartTriggers::TLMC_MoveSyncStartTrigger_Software;

    if (TLMC_MoveSyncStart(hDevice, &moveSyncStartParams) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_PersistParams(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_PersistParams(hDevice, TLMC_ParameterGroupId::TLMC_ParameterGroupId_Unspecified) == TLMC_ResultCodes::TLMC_Success)
    {
...
{
    uint8_t channel1 = 1;
    if (TLMC_RackIdentify(hDevice, channel1) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_RestoreFactoryDefaults(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_RestoreFactoryDefaults(hDevice) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_SendNoFlashProgramming(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_SendNoFlashProgramming(hDevice) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_SendYesFlashProgramming(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_SendYesFlashProgramming(hDevice) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_Stop(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_Stop(hDevice, TLMC_StopModes::TLMC_StopMode_Immediate, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
        // Stage movement has been immediately stopped
    }

    if (TLMC_Stop(hDevice, TLMC_StopModes::TLMC_StopMode_Profiled, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_UnitConverter(const TLMC_DeviceHandle hDevice)
{
    ...
    TLMC_Unit converterUnit;

    if (TLMC_ConvertFromDeviceToPhysical(hDevice, TLMC_ScaleTypes::TLMC_ScaleType_Distance,
        distanceInDeviceUnits, &distanceInPhysicalUnits, &converterUnit) == TLMC_ResultCodes::TLMC_Success)
    {
    ...
    if (TLMC_ConvertFromPhysicalToDevice(hDevice, TLMC_ScaleTypes::TLMC_ScaleType_Distance,
        TLMC_Units::TLMC_Unit_Millimetres, distanceInPhysicalUnits, &distanceInDeviceUnits) == TLMC_ResultCodes::TLMC_Success)
    {
    ...
    if (TLMC_ConvertFromDeviceToPhysical(hDevice, TLMC_ScaleTypes::TLMC_ScaleType_Velocity,
        velocityInDeviceUnits, &velocityInPhysicalUnits, &converterUnit) == TLMC_ResultCodes::TLMC_Success)
    {
    ...
    if (TLMC_ConvertFromPhysicalToDevice(hDevice, TLMC_ScaleTypes::TLMC_ScaleType_Velocity,
        TLMC_Units::TLMC_Unit_Millimetres, velocityInPhysicalUnits, &velocityInDeviceUnits) == TLMC_ResultCodes::TLMC_Success)
    {
    ...
    if (TLMC_ConvertFromDeviceToPhysical(hDevice, TLMC_ScaleTypes::TLMC_ScaleType_Acceleration,
        accelerationInDeviceUnits, &accelerationInPhysicalUnits, &converterUnit) == TLMC_ResultCodes::TLMC_Success)
    {
    ...
    if (TLMC_ConvertFromPhysicalToDevice(hDevice, TLMC_ScaleTypes::TLMC_ScaleType_Acceleration,
        TLMC_Units::TLMC_Unit_Millimetres, accelerationInPhysicalUnits, &accelerationInDeviceUnits) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_PZ_StartOutputWaveform(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_PZ_StartOutputWaveform(hDevice) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_PZ_StopOutputWaveform(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_PZ_StopOutputWaveform(hDevice) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_PZIM_PulseParaAcquire(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_PZIM_PulseParaAcquire(hDevice, TLMC_InfiniteWait) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_GetChannel(const TLMC_DeviceHandle hDevice)
{
    ...
    if (TLMC_GetChannel(hDevice, channelNumber, &channelHandle) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_RegisterLoggingHandler()
{
    if (TLMC_RegisterLoggingHandler(Example_LoggingHandlerCallbackFn, TLMC_LogLevels::TLMC_LogLevel_Information,
        TLMC_LogCategoryFilters::TLMC_LogCategoryFilter_User, 0x1234) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_UnregisterLoggingHandler()
{
    if (TLMC_UnregisterLoggingHandler(Example_LoggingHandlerCallbackFn) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_WritingRawPackets(const TLMC_DeviceHandle hDevice)
{
    unsigned char writeBuffer[6]{ 0x05, 0x00, 0x00, 0x00, 0x50, 0x01 }; // Hardware information request packet

    if (TLMC_WritePacket(hDevice, writeBuffer, sizeof(writeBuffer)) == TLMC_ResultCodes::TLMC_Success)
    {
...
void Example_ReadingRawPackets_GlobalFn(const TLMC_DeviceHandle hDevice)
{
    if (TLMC_RegisterPacketHandler(hDevice, Example_PacketHandler_GlobalFn, 1) == TLMC_ResultCodes::TLMC_Success)
    {
        // The function Example_PacketHandler_GlobalFn has been registered. It will be called
        ...
        if (TLMC_UnregisterPacketHandler(hDevice, Example_PacketHandler_GlobalFn) == TLMC_ResultCodes::TLMC_Success)
        {
            // The callback function Example_PacketHandler_GlobalFn has been unregistered.
            // It will no longer be called.
        }
    }
}
...
void __stdcall Example_PacketHandler_GlobalFn(const unsigned char* pBuffer, const size_t bufferLength, const uintptr_t userData)
{
    ...
    // See Example_ReadingRawPackets_GlobalFn for an example of how to register this function as a callback.
...
{
public:
    void PacketHandlerFn(const unsigned char* pBuffer, const size_t bufferLength)
    {
        // This function will be called by Example_PacketHandler_ObjectFn whenever a packet related to the relevant device
        // handle is received.
        // See Example_PacketHandler_ObjectFn for how this function on this object instance is called.
...
void Example_ReadingRawPackets_ObjectFn(const TLMC_DeviceHandle hDevice)
{
    std::shared_ptr<PacketHandler> myPacketHandler = std::make_shared<PacketHandler>();

    if (TLMC_RegisterPacketHandler(hDevice, Example_PacketHandler_ObjectFn, reinterpret_cast<uintptr_t>(myPacketHandler.get())) == TLMC_ResultCodes::TLMC_Success)
    {
        // We have successfully registered the callback function Example_PacketHandler_ObjectFn. It will be called
        // whenever a packet is received that is related to the device handled by hDevice.

        // Normally, you would leave the callback function registered until immediately before closing the device.
        // Here we unregister immediately for demonstration purposes.
        if (TLMC_UnregisterPacketHandler(hDevice, Example_PacketHandler_ObjectFn) == TLMC_ResultCodes::TLMC_Success)
        {
            // The callback function Example_PacketHandler_ObjectFn has been unregistered.
            // It will no longer be called.
        }
    }
}
...
{
    // This function will be called whenever a packet is received in relation to the device handled by hDevice.
    // See Example_ReadingRawPackets_ObjectFn for an example of how to register this function as a callback.

    // userData is the user-specified value that was passed into TLMC_RegisterPacketHandler. The XA API does not
    // perform any processing on this and simply passes it to the callback handler. You can use this to pass
    // anything you want. In this example, we receive a pointer to a class instance for processing the packet.

    PacketHandler* myPacketHandler = reinterpret_cast<PacketHandler*>(userData);

    if (myPacketHandler != nullptr)
    {
        myPacketHandler->PacketHandlerFn(pBuffer, bufferLength);
    }
}
...
void Example_ReceivingNotifications(const TLMC_DeviceHandle hDevice)
{
    uintptr_t myUserData = hDevice; // myUserData is passed into the registration function. The value
                                    ...
                                    // with the registration and notification. Another is to pass in a
                                    // pointer to a C++ class instance representing the device.
                                    // If your handler doesn't make use of the user data argument, it's a
                                    // good idea to specify zero or nullptr in the registration.

    if (TLMC_RegisterNotificationHandler(hDevice, Example_NotificationHandlerCallbackFn, myUserData) == TLMC_ResultCodes::TLMC_Success)
    {
        ...
        if (TLMC_UnregisterNotificationHandler(hDevice, Example_NotificationHandlerCallbackFn) == TLMC_ResultCodes::TLMC_Success)
        {
...
void __stdcall Example_NotificationHandlerCallbackFn(const TLMC_Notification* pNotification, const uintptr_t userData)
{
    ...
    {
        case TLMC_NotificationIds::TLMC_NotificationId_StatusItemChanged:
            {
                // Status item notifications contain extra data in the 'data' field
                TLMC_StatusItemChangedNotificationData* pExtraData = reinterpret_cast<TLMC_StatusItemChangedNotificationData*>(pNotification->data);

                // TLMC_StatusItemChangedNotificationData has the following fields:
                // count - The number of status items that have been updated
                // ids   - A pointer to a block of memory holding an array of 'TLMC_StatusItemId'
                ...
                // Loop through the status item identifiers that changed as part of this notification
                {
                    ...
                    {
                        TLMC_StatusItemId statusItemId = pExtraData->ids[index];
                        ...
                        {
                            TLMC_DeviceHandle hDevice = static_cast<TLMC_DeviceHandle>(userData);
                            TLMC_StatusItem statusItem;

                            if (TLMC_GetStatusItem(hDevice, statusItemId, &statusItem) == TLMC_ResultCodes::TLMC_Success)
                            {
                                switch (statusItem.valueType)
                                {
                                    case TLMC_ValueTypes::TLMC_ValueType_bool:
                                        // This status item contains a boolean, therefore 'statusItem.value.boolValue'
                                        // holds the value
                                        break;

                                    case TLMC_ValueTypes::TLMC_ValueType_int64:
        ...
        case TLMC_NotificationIds::TLMC_NotificationId_HomeParamsChanged:
            {
                TLMC_DeviceHandle hDevice = static_cast<TLMC_DeviceHandle>(userData);
                TLMC_HomeParams params;

                if (TLMC_GetHomeParams(hDevice, &params, TLMC_NoWait) == TLMC_ResultCodes::TLMC_Success)
                {
                    // The new homing parameters have been copied into the local 'params' buffer
                }
            }
            break;

        // Here we process receiving rich response notifications
        case TLMC_NotificationIds::TLMC_NotificationId_RichResponseChanged:
            {
                TLMC_DeviceHandle hDevice = static_cast<TLMC_DeviceHandle>(userData);
                struct TLMC_RichResponse richResponse;

                if (TLMC_GetRichResponse(hDevice, &richResponse) == TLMC_ResultCodes::TLMC_Success)
                {
...
void Example_SimulationCreateAndRemove()
{
    ...
    TLMC_ResultCode resultCode = TLMC_CreateSimulation(pSimulationDescription);
    ...
    if (resultCode == TLMC_ResultCodes::TLMC_Success)
    {
        ...
        resultCode = TLMC_Open("104003335", NULL, TLMC_OperatingModes::TLMC_OperatingMode_Default, &hDevice);
