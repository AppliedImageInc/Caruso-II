...
def line_intersection(x1, y1, x2, y2, x3, y3, x4, y4):
    ...
    if 0 <= t <= 1 and 0 <= u <= 1:
        ...
        return None  # The intersection point is not within the line segments
...
def cpoint(xy,el):
	...
	if xy == 'i' or xy == 2:
		tpx = lx + linecenter(edges[0])
		tpy = ly + 222*mmpp

		bpx = lx + linecenter(edges[1])
		bpy = ly - 222*mmpp

		lpx = lx - 222*mmpp
		lpy = ly - linecenter(edges[2])

		rpx = lx + 222*mmpp
		rpy = ly - linecenter(edges[2])

		cx, cy = line_intersection(tpx,tpy,bpx,bpy,lpx,lpy,rpx,rpy)
...
def measurepart():
	...
	for i in range(len(xy)):
		...
		points.append([name + 'x1', str(xc - r), str(yc),'x','e'])
		points.append([name + 'x2', str(xc + r), str(yc),'x','e'])
		points.append([name + 'y1', str(xc), str(yc - r),'y','e'])
		points.append([name + 'y2', str(xc), str(yc + r),'y','e'])
	...
	for i in range(len(diam)):
		...
		for j in range(len(points)):
			edgedata[j,0], edgedata[j,1] = cpoints[points[i]]
		print(edgedata)
